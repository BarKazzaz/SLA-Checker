    ...
    if (notifId in myNotificationIDs) {
        if (btnIdx == 0) //list tickets btn
        {
            ...
            chrome.storage.sync.get('snoozeTime', function(data) {
                if (data.snoozeTime) { snoozeTime = data.snoozeTime; } else { snoozeTime = 15; }
...
function snoozePerTab(notificationId) {
    ...
    if (notificationId == 'setPriority') { chrome.tabs.create({ 'url': 'https://incapsula.zendesk.com/agent/filters/39491500', 'active': true }); }
    ...
    chrome.storage.sync.get('mute', function(data) {
        ...
        if (shouldMute) {
            ...
            chrome.storage.sync.get('myVolume', function(data) {
                if (data.myVolume) {
                    notificationVolume = data.myVolume / 100.0;
                } else {
...
chrome.storage.sync.get(['monitorInterval', 'snoozeTime', 'tierOneLink'], function(data) {
    if (data.monitorInterval) { monitorInterval = data.monitorInterval; } else {
        monitorInterval = DEFAULT_MONITOR_VALUE;
        chrome.storage.sync.set({ 'monitorInterval': monitorInterval })
    }

    if (data.snoozeTime) { snoozeTime = data.snoozeTime; } else {
        snoozeTime = DEFAULT_SNOOZE_TIME;
        chrome.storage.sync.set({ 'snoozeTime': snoozeTime })
    }

    if (data.tierOneLink) { tierOneLink = data.tierOneLink; } else {
        tierOneLink = DEFAULT_TIER_ONE_LINK;
        chrome.storage.sync.set({ 'tierOneLink': tierOneLink })
    }
